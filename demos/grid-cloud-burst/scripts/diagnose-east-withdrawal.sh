...
capture_state() {
  local label="$1" now desired ready endpoints overlay rev phase
  now="$(date -u +%FT%T.%3NZ)"
  ...
  rev="$(jq -r '.revision.value // empty' <<<"$overlay")"
...
traffic_loop() {
  local seq=0 start body status rid at
  while :; do
