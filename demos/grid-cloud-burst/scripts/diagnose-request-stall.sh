...
ui() { curl -ksS --connect-timeout 5 --max-time 35 -u praxis:redhat123 "$@"; }
...
traffic() {
  local n=0 id start end start_ns end_ns body_file timing_file body status
  while :; do
    n=$((n+1)); id="stall-${STARTED}-${n}"; start="$(date -u +%FT%T.%3NZ)"; start_ns="$(date +%s%N)"
